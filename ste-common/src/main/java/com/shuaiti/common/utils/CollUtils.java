...
import com.shuaiti.common.validate.Checker;
import java.util.*;
...
public class CollUtils extends CollectionUtil {
    ...
    public static List<Integer> convertToInteger(List<String> originList){
        return CollUtils.isNotEmpty(originList) ? originList.stream().map(Integer::parseInt).collect(Collectors.toList()) : null;
    }

    public static List<Long> convertToLong(List<String> originLIst){
        return CollUtils.isNotEmpty(originLIst) ? originLIst.stream().map(Long::parseLong).collect(Collectors.toList()) : null;
    }
